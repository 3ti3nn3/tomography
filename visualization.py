...
def infidelity(rho_0: np.array, func1, func2, M1: np.array, M2: np.array, N_max: int, n_mean: int, q=False):
    '''
    ...
    :param N_max : maximal N
    :param n_mean: number of mean measurements for mean value
    ...
    rho_m1  = np.zeros((n_mean, n_steps, dim, dim), dtype=np.complex)
    rho_m2  = np.zeros((n_mean, n_steps, dim, dim), dtype=np.complex)
    inf_m1  = np.zeros((n_mean, n_steps), dtype=np.float)
    inf_m2  = np.zeros((n_mean, n_steps), dtype=np.float)
    ...
    # axs[0].set_xlim(100, N_max)
    # axs[1].set_xlim(100, N_max)
    # axs[2].set_xlim(100, N_max)
