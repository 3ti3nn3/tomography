...
class OneStepTomography(Tomography):
    ...
    def __init__(self, name, dim=None, N_max=None, N_mean=None, N_ticks=None, povm_name=None, f_sample=None, f_estimate=None, f_distance=None, new=True, debug=False):
        ...
        # reload data
        if self.new:
            notNone = [dim is not None, N_max is not None, N_mean is not None, N_ticks is not None,\
            ...
            with open('data/'+self.name+'.pt', 'rb') as file:
                ...
                self.dim     = ost.dim
                self.N_max   = ost.N_max
                self.N_mean  = ost.N_mean
                self.N_ticks = ost.N_ticks
                self.x_N     = np.logspace(2, np.log10(self.N_max), self.N_ticks, dtype=np.int)
    ...
    def reconstruct(self):
        ...
        for j in range(self.N_mean):
            measurement = simulate.measure(self._originals[j], self.N_max, self.povm)
...
class OneStepComparison(Comparison):
    ...
    def __init__(self, name, name_list, debug=False):
        ...
        assert all([ost_ref.N_max == ost.N_max for ost in self.tomo_list]), 'Different N_max encountered. Comparison does not make sense!'
